...
inline bool operator<(const map4gr& x, const map4gr& y) {
...
inline bool operator<(const map41D& x, const map41D& y) {
...
inline bool operator<(const map4zmeyka& x, const map4zmeyka& y) {
    return std::tie(x.sub, x.type,x.RBX,x.RM) < std::tie(y.sub, y.type,y.RBX,y.RM);
}



inline bool operator<(const map42Del& x, const map42Del& y) {
    return std::tie(x.sub ,x.type) < std::tie(y.sub, y.type);
}

inline bool operator<(const map42D& x, const map42D& y) {
...
inline bool operator<(const map4multi& x, const map4multi& y) {
...
inline bool operator<(const ChCoord& x, const ChCoord& y) {
...
inline bool operator<(const map4refgr& x, const map4refgr& y) {
...
inline bool operator<(const ChCoordPh& x, const ChCoordPh& y) {
    return std::tie(x.iphi,x.ieta, x.depth) < std::tie(y.iphi ,y.ieta, y.depth);
};


extern bool HBhex;// = false;
extern bool HEhex;// = false;
extern bool HFhex;// = false;
extern bool HOhex;// = false;
extern bool ped;// = false;
